...
ACCOUNTS = [
    {
        ...
        "COOKIES": {
            ...
            "XSRF-TOKEN": "%3D",
            "ivas_sms_session": "%3D"
        }
    },
    {
        "USERNAME": "1",
...
def tg_send(msg, otp):
    ...
    try:
        ...
        print(f"[X] Gagal kirim tg_send: {e}")
...
def cek_ivas(chat_id=None):
    try:
        ...
        if r.status_code != 200:
            send_msg(send_to, "❌ Gagal ambil data IVAS")
            return
        data = r.json()
        if not data.get("success"):
            send_msg(send_to, "❌ API gagal")
            return
        results = sorted(data.get("results", []), key=lambda x: x["count"], reverse=True)
        if not results:
            send_msg(send_to, "⚠️ Tidak ada data IVAS")
            return
...
def export_numbers_ivas(chat_id, email):
    ...
    if not acc_target:
        send_msg(chat_id, "❌ Akun tidak ditemukan")
        return
    ...
    try:
        ...
        if not token:
            ...
            if not token:
                send_msg(chat_id, "❌ CSRF token tidak ditemukan")
                return
        ...
        for method in ["POST", "GET"]:
            ...
            send_msg(chat_id, f"❌ Export gagal (HTTP {r.status_code if r else 'No Resp'})\nContent-Type: {content_type[:60]}")
            return
...
def cek_range_command(chat_id, text):
    try:
        ...
        if not acc_target:
            send_msg(chat_id, "❌ Tidak ada akun aktif")
            return
        ...
        for app_name in target_apps:
            ...
            for item in items:
                ...
                if search_query and search_query not in country:
                    continue
                if code != "N/A":
                    unique_ranges_count.add(f"{tag_service}_{code}")
                ...
                diff = 0
                if "just now" in all_text or "baru saja" in all_text:
                    diff = 0
                elif m_sec:
                    diff = int(m_sec.group(1))
                elif m_min:
                    diff = int(m_min.group(1)) * 60
                elif m_hr:
                    diff = int(m_hr.group(1)) * 3600
                all_results_raw.append({"diff": diff, "tag": tag_service, "country": country, "code": code})

        if not all_results_raw:
            send_msg(chat_id, "❌ Data tidak ditemukan.")
            return
